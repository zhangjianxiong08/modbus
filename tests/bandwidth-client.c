 ...
 * @Author: your name
 * @Date: 2019-08-06 17:12:07
 * @LastEditTime: 2019-08-16 17:32:31
...
int main(int argc, char *argv[])
{
    ...
    uint16_t tmp = 0;
    //start = gettime_ms();
    //modbus_write_register(ctx, 1999, 8);
    int i2 =0x1;
    while(1)
    {
        rc = modbus_read_registers(ctx, i2, 1, &tmp);
        if(-1 == rc)
            break;
        i2++;
    }
    ...
    printf("read ret = %d, result = %d, i2=%d\n", rc, tmp, i2);
    return 1;
    end = gettime_ms();
    elapsed = end - start;
